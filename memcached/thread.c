...
void memcached_thread_init(int nthreads, void *arg) {
	...
	/* Initialize merkle tree here */
	fprintf(stderr, "[SEVAULT]\tInitializing Merkle Tree...\t\t\t");
	merkle = mt_create();
	if (merkle == NULL)
		fprintf(stderr, "\n[SEVAULT]\tFailed to initialize Merkle Tree... :(\n");
	fprintf(stderr, "OK\n");
    ...
    for (i = 0; i < nthreads; i++) {
		...
		/* Set Merkle tree pointer */
		threads[i].mt_ptr = merkle; 
    }
