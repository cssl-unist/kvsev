...
/* Key to calculate CMAC */
const unsigned char cmac_key[16] = { 0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae,
...
void complete_nread_ascii(conn *c) {
    ...
    /*
     * Calculate and store CMAC
     */
#ifdef ENABLE_SEVAULT
    unsigned char *cmac_input = (unsigned char *)ITEM_data(it);
    int data_size = it->nbytes;
    AES_CMAC(cmac_key, cmac_input, data_size, it->mac);

    /* DEBUG fprintf */
#ifdef DEBUG_SEVAULT
    for (int i = 0; i < 16; i++) {
        fprintf(stderr, "%02x", it->mac[i]);
		if ((i % 4) == 3)
			fprintf(stderr, " ");
    }
    fprintf(stderr, "\n");
     ...
     * Update Merkle Tree
     * TODO
     * - Different update for SET/UPDATE requests (only for initial SET for now)
     * - Not sure how to handle the index for now
     *   => Will cause problems later on when handling multi-thread
     */
#ifdef ENABLE_SEVAULT
	pthread_mutex_lock(&c->thread->mt_ptr->mutex);
    mt_error_t r = mt_add(merkle, (uint8_t *)it->mac, 16);
    if (r != MT_SUCCESS)
        fprintf(stderr, "[SEV]\tFailed to update Merkle Tree!\n");

    //mt_print(merkle);
    //fprintf(stderr, "[SEV]\ttotal items: %ld\n", stats.total_items);
    it->mt_index = (uint32_t)stats.total_items - 1; /* Merkle Tree index starts from 0, but total_items is already updated at this stage */
	pthread_mutex_unlock(&c->thread->mt_ptr->mutex);
#endif
...
static inline void process_get_command(conn *c, token_t *tokens, size_t ntokens, bool return_cas, bool should_touch) {
    ...
    do {
        while(key_token->length != 0) {
            ...
            /*
             * Verify Merkle Tree on GET request
             */
            if (it) {
                /*
...
#ifdef ENABLE_SEVAULT
				pthread_mutex_lock(&c->thread->mt_ptr->mutex);
                mt_error_t r = mt_verify(merkle, it->mac, 16, it->mt_index);
                if (r != MT_SUCCESS)
                    fprintf(stderr, "[SEV]\tMerkle Tree veriification failed!\n");
				pthread_mutex_unlock(&c->thread->mt_ptr->mutex);
#endif
                {
